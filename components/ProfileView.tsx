...
import { LogOut, Clock, Calendar, Settings, ChevronRight, Users, PlusCircle, LayoutGrid, Bookmark, Activity } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'posted' | 'saved' | 'history'>('posted');
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center pb-24 animate-blur-in">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Guest Account</h2>
  ...
  return (
    <div className="min-h-screen pb-24 animate-blur-in">
      ...
      <div className="px-6 grid grid-cols-3 gap-3 mb-8">
        ...
        <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-center">
            <div className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{history.filter(h => h.type === 'itinerary').length}</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Plans</div>
        </div>
      </div>
      ...
      <div className="px-6 mb-6">
          <div className="flex bg-slate-200 dark:bg-slate-900/50 p-1 rounded-xl">
              <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'posted' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}
              >
            ...
            <div className="space-y-4 animate-slide-up-fade">
                {createdEvents.length === 0 ? (
                     <button 
                        ...
                        className="w-full py-12 bg-slate-100 dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 rounded-xl text-slate-500 flex flex-col items-center justify-center gap-3 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors group"
                    >
                        ...
                        <span className="font-medium">Post your first event</span>
                    </button>
                ) : (
                    <div className="grid grid-cols-1 gap-4">
                        {createdEvents.map(event => (
                           <div key={event.id} className="bg-white dark:bg-slate-800 rounded-xl p-3 flex gap-4 border border-slate-200 dark:border-slate-700 shadow-sm">
